  ...
  return (
    <Layout>
      ...
      <div className="min-h-screen bg-[var(--rice-white)]">
        {/* Header Section */}
        <div className="bg-gradient-to-b from-[var(--dzong-blue)] to-[var(--river-blue)] pt-32 pb-20">
          <div className="container mx-auto px-4">
            <h1 className="text-4xl md:text-5xl font-bold text-center mb-6 font-['Cinzel'] text-white leading-tight animate-fadeIn">
              Essential Travel Tips for Bhutan
            </h1>
            <div className="h-1 w-32 bg-[var(--rice-white)] mx-auto mb-6 rounded-full"></div>
            <p className="text-lg md:text-xl text-center font-['Playfair_Display'] text-[var(--rice-white)] max-w-2xl mx-auto">
              Your guide to experiencing the Kingdom of Happiness like a local
            </p>
          </div>
        </div>

        {/* Category Filter */}
        <div className="bg-white shadow-lg relative z-10 -mt-8 mx-4 rounded-xl">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {categories.map(category => (
                <button
                  ...
                  className={`px-6 py-3 rounded-lg transition-all duration-300 font-['Cinzel'] text-base
                    ${selectedCategory === category
                      ? 'bg-[var(--dzong-blue)] text-white shadow-lg transform scale-105'
                      : 'bg-white text-[var(--dzong-blue)] border-2 border-[var(--dzong-blue)] hover:bg-[var(--rice-white)]'
                    }`}
                >
                  {category}
                </button>
              ...
              {filteredTips.map((tip, index) => (
                ...
                >
                  <div className="flex items-center mb-6">
                    <div className="w-12 h-12 rounded-full bg-[var(--rice-white)] flex items-center justify-center mr-4">
                      <i className={`bi bi-${tip.icon} text-2xl text-[var(--river-blue)]`}></i>
                    </div>
                    <h3 className="text-xl font-bold font-['Cinzel'] text-[var(--dzong-blue)]">
                      {tip.title}
                    </h3>
                  </div>
                  <div className="bg-[var(--rice-white)] p-6 rounded-lg">
                    <p className="text-[var(--mountain-slate)] font-['Playfair_Display'] leading-relaxed">
                      {tip.content}
                    </p>
                  </div>
                  <div className="mt-4 flex justify-end">
                    <span className="text-sm font-medium text-[var(--river-blue)] bg-[var(--rice-white)] px-3 py-1 rounded-full">
                      {tip.category}
                    </span>
                  </div>
                </div>
